...
class LHEHand:
    ...
    def __init__(self, sb_value: int, players_in: List[Player]):
        ...
        self.turn_index = (self.sb_index + 2) % len(players_in)
    ...
    def play_round(self, round: int):
        ...
        if round == 0: # First round is special
            self.turn_index = (self.turn_index + 2) % len(self.players_in)
            self.num_raises = 1
    ...
    def perform_action(self, action, player):
        ...
        if self.num_raises == 4 and action == 2:
            action = 1
    ...
    def check_all_calls(self, current_round_actions):
        ...
        calls_raises = list(filter(lambda x: x.action != 0, current_round_actions))
